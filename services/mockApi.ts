...
let DB = {
  ...
  keys: [
    ...
    {
      id: "key-03",
      keyValue: "AGENT-BOB-456",
      type: "agent",
      status: "suspended",
      createdAt: new Date(Date.now() - 5 * 24 * 3600 * 1000).toISOString(),
      expiresAt: new Date(Date.now() + 60 * 24 * 3600 * 1000).toISOString(),
      agentId: "user-agent-02",
      note: "Key for Bob",
    },
    {
      id: "key-04",
      keyValue: "AGENT-EXPIRED-789",
      type: "agent",
      status: "expired",
  ...
  customers: [
    {
      ...
      avatarUrl: "https://picsum.photos/seed/george/100",
    },
    {
      id: "cust-02",
      name: "Jane Doe",
  ...
  chatMessages: [
    ...
    },
    {
      ...
      senderId: "cust-01",
      senderType: "customer",
      content: "It's #12345. I received the wrong item.",
      type: "text",
    ...
    },
    {
      ...
      senderId: "system",
      senderType: "system",
      content: 'File "receipt.png" uploaded.',
      type: "file",
  ...
  agentSettings: [
    {
      ...
      welcomeMessages: [
        {
          id: "wm-1",
          message: "Welcome to our support chat! How can I assist you?",
          delaySeconds: 2,
        },
      ],
      blacklist: [
...
export const api = {
  ...
  async createKey(data: {
    ...
    const newKey: Key = {
      id: `key-${Date.now()}`,
      keyValue: `${data.type.toUpperCase()}-${Math.random()
        .toString(36)
        .substring(2, 10)
        .toUpperCase()}`,
      type: data.type,
  ...
  async getAgentDashboardData(
    ...
    const customers = DB.customers.filter((c) =>
      agentSessions.some((s) => s.customerId === c.id)
    );
    const messages = DB.chatMessages.filter((m) =>
      agentSessionIds.includes(m.sessionId)
    );
  ...
  async getNewMessages(
    ...
    await networkDelay(200);
    // 获取该会话的所有消息
    const sessionMessages = DB.chatMessages.filter(
      (m) => m.sessionId === sessionId
    );
    // 返回比当前数量多的新消息
    return sessionMessages.slice(currentCount);
  },

  async getVisitorChatData(
    shareId: string
  ): Promise<{
    agent: Agent;
    customer: Customer;
    ...
    let session = DB.chatSessions.find(
      (s) => s.agentId === agent.id && s.status === "pending"
    );
    ...
    if (session) {
      ...
      customer = {
        ...
        avatarUrl: `https://picsum.photos/seed/${customerId}/100`,
      };
    ...
    if (senderType === "customer") {
      setTimeout(() => {
        ...
        const agentReply: ChatMessage = {
          ...
          senderType: "agent",
          content: `I've received your message: "${content.substring(
            0,
            30
  ...
  async updateAgentSettings(
    ...
    DB = produce(DB, (draft) => {
      const settingsIndex = draft.agentSettings.findIndex(
        (s) => s.agentId === agentId
      );
      if (settingsIndex !== -1) {
        draft.agentSettings[settingsIndex] = {
          ...draft.agentSettings[settingsIndex],
          ...data,
        };
